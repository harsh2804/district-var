...
def getdata(district):
    ...
    #a = []
    df_ = pd.read_csv('district_mod/' + district+".csv")
    d = df_['name'][0]
    ...
    dff = df_.set_index('year').stack().reset_index(name='rain') #make everything in one column
    dff=dff.rename(columns={"level_1": "month"})
    dff['year'] = pd.to_numeric(dff['year'],downcast='integer')
    for i in range(len(dff)):
     mon_num = strptime(dff['month'][i],'%b').tm_mon
     dff['month'][i] = mon_num
    dff['name'] = d
    return dff
...
s4 = pn.widgets.Select(name = 'District',options =district,width = 100,value='AGRA')
k = s4.value
k1 = b[b.name == k]
m1 = int(k1.year.min())
m2 = int(k1.year.max())
...
def p3(s4): #,s1,s2,s3,s4):
      # await asyncio.sleep(2)
       ...
       for i in range(len(a)) :
         df1 =  b[b['month'].isin(a[i])] #  dff[(dff.month == 1) | (dff.month == 2)]
         df1 = df1.groupby('year').sum()
         df1 = df1.reset_index()
         df1.columns = ['year',g[i]]
         v.append(df1)
       b = pd.concat(v,axis = 1) 
       b = b.loc[:,~b.columns.duplicated()]
       b['name'] = s4
       b = b.set_index(['year','name']).stack().reset_index()
       #a = pd.read_excel('subdivision_data_1901-2019 _m1.xlsx',engine='openpyxl')
...
def p4(s4):#,s1,s2,s3,s4):
    #   await asyncio.sleep(2)
       ...
       b = b.loc[:,~b.columns.duplicated()]
       b['name'] = s4
       #a = pd.read_csv(s4+'.csv',engine='openpyxl')
       #a = a[['year','name','JF','MAM','JJAS','OND']]
       #b = a.set_index(['year','name']).stack().reset_index()
